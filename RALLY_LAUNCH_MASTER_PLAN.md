...
- [ ] [C] Audit Amber's current state: rp_identities doc, LoRA training runs, reference images, past output quality
- [ ] [C] Benchmark: Amber via current providers vs 2026 VTON/identity landscape (what's shipped since the I-series was built) on OUR garments — side-by-side renders
- [ ] [C] Economics memo: virtual model vs. stock/model photography per product; consistency value for brand (same face across 1000s of products = brand asset)
- [ ] [G] **GO/NO-GO gate** — decision with benchmark renders in hand
