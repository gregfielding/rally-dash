...
async function composeStageA({ db, storage, sharp, functions, input }) {
  ...
  try {
    ...
    /**
     * Model targets are clipped to the garment silhouette AFTER the quad warp
     * (in model-photo space) by the dedicated block below — applying a mask
     * here in flat pre-warp space would clip the wrong region. So skip the
     * pre-warp multiply for model targets; flat targets keep it.
     */
    const maskDoc =
      maskMode === "none" || isModelTarget
        ? null
        : await db.collection("rp_blank_masks").doc(maskDocId).get();
  ...
  /**
   * Phase L7 (2026-06-02): garment-silhouette clip for model targets.
   *
   * Clips the (warped or flat) design to the visible-fabric mask IN MODEL-PHOTO
   * SPACE — so any print that extends past the fabric edge (armhole, side seam,
   * neckline) is cut, making the design curve out of sight at the body's edge
   * instead of painting onto skin or the background. Runs AFTER the warp and
   * BEFORE the letter mask, so both the visible print AND Flux Fill's inpaint
   * region are bounded by the fabric.
   *
   * Only alpha is multiplied (not RGB) + the mask is feathered, so the edge
   * fades softly to transparent (curving away) rather than a hard cookie-cut.
   *
   * Requires the per-pose model mask (`{blankId}_{variantId}_model_<view>`,
   * generated by "Generate AI mask for model front/back"). No mask → no-op
   * (legacy behavior); the editor badge already warns when one is missing.
   */
  let garmentClipApplied = false;
  if (isModelTarget) {
    try {
      const garmentMaskDocId = maskDocIdForTarget(blankId, view, renderTarget, variantId);
      const garmentMaskDoc = await db.collection("rp_blank_masks").doc(garmentMaskDocId).get();
      const gmData = garmentMaskDoc.exists ? garmentMaskDoc.data() : null;
      if (gmData && gmData.mask && gmData.mask.downloadUrl) {
        const gmResp = await fetch(gmData.mask.downloadUrl);
        if (gmResp.ok) {
          /** Feather ~0.4% of canvas width so the fabric edge softly falls off. */
          const featherSigma = Math.max(1, Math.round(blankWidth * 0.004));
          /**
           * Resize the silhouette to full canvas, feather, then extract the
           * region matching composeDesignRaw's footprint. For the warped path
           * that's the full canvas (composeLeft/Top = 0); for the flat path
           * it's the placement rect — both are described by composeLeft/Top/W/H.
           */
          const gmRaw = await sharp(Buffer.from(await gmResp.arrayBuffer()))
            .resize(blankWidth, blankHeight, { fit: "fill" })
            .grayscale()
            .blur(featherSigma)
            .extract({ left: composeLeft, top: composeTop, width: composeW, height: composeH })
            .raw()
            .toBuffer({ resolveWithObject: true });
          const gm = gmRaw.data; // 1 byte/px, composeW × composeH
          const gmPixels = gm.length;
          let gmSum = 0;
          for (let p = 0; p < gmPixels; p++) gmSum += gm[p];
          const gmMean = gmPixels > 0 ? gmSum / gmPixels : 0;
          /** Skip near-empty silhouettes (mask failure) — would erase the whole design. */
          if (gmMean >= 5) {
            for (let p = 0; p < gmPixels; p++) {
              const a = p * 4 + 3;
              composeDesignRaw[a] = Math.round((composeDesignRaw[a] * gm[p]) / 255);
            }
            garmentClipApplied = true;
            maskApplied = true;
            console.log(
              `[composeStageA] Phase L7 garment-silhouette clip applied (${renderTarget}, variant ${variantId}, featherσ=${featherSigma})`
            );
          } else {
            console.log(
              `[composeStageA] garment clip skipped (silhouette mean=${Math.round(gmMean)} < 5 — mask likely empty/failed)`
            );
          }
        }
      }
    } catch (clipErr) {
      console.warn("[composeStageA] garment silhouette clip failed:", clipErr && clipErr.message);
    }
  }
  ...
  return {
    stageA: {
       ...
       *  (curves out of sight at the fabric edge instead of painting on skin). */
      garmentClipApplied,
